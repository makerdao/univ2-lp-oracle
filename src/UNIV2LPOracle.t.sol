...
contract SeekableOracle is UNIV2LPOracle {
    constructor(
        ...
        address _orb0,
        address _orb1
    ) public UNIV2LPOracle(_src, _wat, _orb0, _orb1) {}
...
contract UNIV2LPOracleTest is DSTest {
    ...
    address constant DAI_ETH_UNI_POOL =
        0xA478c2975Ab1Ea89e8196811F51A7B7Ade33eB11;
    address constant WBTC_ETH_UNI_POOL =
    ...
    address constant UNISWAP_ROUTER_02 =
    ...
    function setUp() public {
        ...
        hevm.store(
            ...
            bytes32(
                uint256(
                    43353987475243871752608912172418385489740068120774565181786433903143712498119
                )
            )
        );
        hevm.store(
            ...
            bytes32(uint256(55130522579388813557146055))
        );
        hevm.store(
            WETH,
            keccak256(abi.encode(DAI_ETH_UNI_POOL, uint256(3))), // WETH balance of DAI_ETH pool
            bytes32(uint256(94328066153704376274664))
        );
        hevm.store(
            WBTC_ETH_UNI_POOL,
            0, // totalSupply
        ...
        hevm.store(
            ...
            bytes32(
                uint256(
                    43353988796281258443110612805302462270865328117852610699179961909467622877495
                )
            )
        );
        hevm.store(
        ...
        seekableOracleDAI = new SeekableOracle(
            DAI_ETH_UNI_POOL,
            poolNameDAI,
            USDC_ORACLE,
            ETH_ORACLE
        ...
        seekableOracleWBTC = new SeekableOracle(
            WBTC_ETH_UNI_POOL,
            poolNameWBTC,
            WBTC_ORACLE,
            ETH_ORACLE
        ...
        IERC20(WETH).approve(UNISWAP_ROUTER_02, uint256(-1)); // Approve WETH to trade on Uniswap

        hevm.store(
            DAI,
            keccak256(abi.encode(address(this), uint256(2))),
            bytes32(uint256(10_000 ether))
        ); // Mint 10,000 DAI
        IERC20(DAI).approve(UNISWAP_ROUTER_02, uint256(-1)); // Approve DAI to trade on Uniswap
        ...
        IERC20(WBTC).approve(UNISWAP_ROUTER_02, uint256(-1)); // Approve WBTC to trade on Uniswap
    }
    ...
    function testFail_build_invalid_pool() public {
        factory.build(address(0), poolNameDAI, WBTC_ORACLE, ETH_ORACLE); // Attempt to deploy new LP oracle
    }

    function testFail_build_invalid_oracle() public {
        factory.build(DAI_ETH_UNI_POOL, poolNameDAI, WBTC_ORACLE, address(0)); // Attempt to deploy new LP oracle
    }

    function testFail_build_invalid_oracle2() public {
        factory.build(DAI_ETH_UNI_POOL, poolNameDAI, address(0), ETH_ORACLE); // Attempt to deploy new LP oracle
    }
    ...
    function test_compare_sqrt(uint256 exp) public {
        ...
        uint256 babylGas = preGas - postGas;
        uint256 altGas = preAltGas - postAltGas;
        ...
        assertTrue(
            wdiv(mul(babylGas, WAD), mul(altGas, WAD)) > mul(4, WAD) || exp == 0
        );
    ...
    function test_seek_equivalence(
        ...
        uint256 k_alt = mul(normalize_product, mul(res00, res11));
    ...
    function test_seek_dai() public {
        ...
        uint256 expectedPrice =
            add(
                mul(ethPrice, IERC20(WETH).balanceOf(DAI_ETH_UNI_POOL)),
    ...
    function test_seek_wbtc() public {
        ...
        uint256 expectedPrice =
            add(
                mul(ethPrice, IERC20(WETH).balanceOf(WBTC_ETH_UNI_POOL)),
                mul(
                    wbtcPrice,
                    IERC20(WBTC).balanceOf(WBTC_ETH_UNI_POOL) * 10**10
                )
    ...
    function test_seek_internals() public {
        ...
        if (ERC20Like(tok0).decimals() < 18) {
            // Check if token0 has non-standard decimals
            res0 = uint112(
                res0 * 10**(18 - uint256(ERC20Like(tok0).decimals()))
            ); // Adjust reserves of token0
        }
        if (ERC20Like(tok1).decimals() < 18) {
            // Check if token1 has non-standard decimals
            res1 = uint112(
                res1 * 10**(18 - uint256(ERC20Like(tok1).decimals()))
            ); // Adjust reserves of token1
        }
        /*** BEGIN TEST 2 ***/
        ...
        uint256 val0 = OracleLike(wbtcEthLPOracle.orb0()).read(); // Query token0 price from oracle (WAD)
        uint256 val1 = OracleLike(wbtcEthLPOracle.orb1()).read(); // Query token1 price from oracle (WAD)
        ...
        uint256 supply = ERC20Like(WBTC_ETH_UNI_POOL).totalSupply(); // Get LP token supply
        ...
        uint128 quote =
            uint128(mul(2 * WAD, sqrt(wmul(k, wmul(val0, val1)))) / supply); // Calculate LP token price quote
    ...
    function test_double_poke() public {
        ...
        assertEq(uint256(nxtHas), 1); // Verify oracle has queued value
        ...
        assertEq(uint256(curVal), uint256(nxtVal)); // Verify queued value became current value
    ...
    function test_eth_dai_price_change() public {
        ...
        // amounts[1] -> DAI
        diff = amounts[1] > 10_000 ether
            ? amounts[1] - 10_000 ether
            : 10_000 ether - amounts[1]; // Calculate difference between trade and $1 peg
        assertTrue((diff * WAD) / amounts[1] < 0.02 ether); // Verify less than 2% slippage
    ...
    function test_eth_wbtc_price_change() public {
        ...
        assertTrue(
            firstVal < 900_000_000 ether && firstVal > 600_000_000 ether
        ); // 704030123759222892060867448 at time of test
        assertTrue(has); // Verify Oracle has valid value
        ...
        uint256 secondVal = uint256(val); // Cast queued price as uint256
        assertTrue(
            secondVal < 900_000_000 ether && secondVal > 600_000_000 ether
        ); // 704030254653978027824526079 at time of test
        ...
        diff = amounts[1] > (10_000 ether * 1E8) / wbtcPrice
        ...
        uint256 thirdVal = uint256(val); // Cast queued price as uint256
        assertTrue(
            thirdVal < 900_000_000 ether && thirdVal > 600_000_000 ether
        ); // 704030385156122678606782694 at time of test
        assertTrue(has); // Verify Oracle has valid value
    ...
    function testCurSlot0x3() public {
        ...
        bytes32 curPacked =
            hevm.load(address(daiEthLPOracle), bytes32(uint256(3)));
    ...
    function testNxtSlot0x4() public {
        ...
        bytes32 nxtPacked =
            hevm.load(address(daiEthLPOracle), bytes32(uint256(4)));
